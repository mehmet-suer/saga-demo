...
public class OutboxEventScheduler {
    ...
    public OutboxEventScheduler(ProcessedEventService processedEventService,
            @Qualifier(ExecutorConstants.OUTBOX_EXECUTOR) ExecutorService outboxExecutor,
            ...
            try {
                outboxExecutor.submit(() -> processedEventService.process(event)
                        .whenComplete((result, exception) -> {
                            try {
                                logIfExceptionExists(event, exception);
                            } finally {
                                inflight.release();
                            }
                        }));
    ...
    private void logIfExceptionExists(ProcessedEvent event, Throwable exception) {
        if (exception != null) {
